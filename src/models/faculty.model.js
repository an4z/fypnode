...
Faculty.updateFaculty = (id, facultyReqData, result)=>{
    dbConn.query("UPDATE faculty SET first_name=?,last_name=?,email=?,phone=?,organization=?,designation=?,salary=? WHERE id = ?", [facultyReqData.first_name,facultyReqData.last_name,facultyReqData.email,facultyReqData.phone,facultyReqData.organization,facultyReqData.designation,facultyReqData.salary, id], (err, res)=>{
        if(err){
...
Faculty.deleteFaculty = (id, result)=>{
    dbConn.query('DELETE FROM faculty WHERE id=?', [id], (err, res)=>{
        if(err){
